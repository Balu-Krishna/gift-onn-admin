...
@Component({
  selector: "app-product-launch-actions",
  templateUrl: "./product-launch-actions.component.html",
  styleUrls: ["./product-launch-actions.component.scss"],
})
export class ProductlaunchActionsComponent implements OnInit {
  ...
  types = "";
  constructor(
    public dialogRef: MatDialogRef<ProductlaunchActionsComponent>,
  ...
  submitForm() {
    ...
    else
    {
      ...
      const url = (this.data.name=='Deny')?`${this.constants.PRODUCT_LAUNCH_DENY}${this.data.data.id}`:`${this.constants.PRODUCT_LAUNCH_HOLD}${this.data.data.id}`;
      let headers = {
